...
class TrialTask(RLTask):
    ...
    def set_up_scene(self, scene) -> None:
        ...
        self.sphere_color = torch.tensor([0.1, 0.9, 0.1], device=self._device)
        self.sphere_position = torch.tensor([0.5, 0., 0.7], device=self._device)
        self.sphere_lower_bound = torch.tensor([0.2, -0.5, 0.5], device=self._device)
        self.sphere_upper_bound = torch.tensor([0.8, 0.5, 1.], device=self._device)
    ...
    def get_target_sphere(self):
        ...
        sphere.set_collision_enabled(False)
    ...
    def get_observations(self) -> dict:
        ...
        hand_pos_world,  = self._hands.get_world_poses(clone=False)
        target_pos_world, _ = self._spheres.get_world_poses(clone=False)
    ...
    def reset_idx(self, env_ids):
        ...
        pos = tensor_clamp(
            self.sphere_position.unsqueeze(0)
            + 0.25 * (torch.rand((len(env_ids), 3), device=self._device) - 0.5),
            self.sphere_lower_bound,
            self.sphere_upper_bound,
        )
    ...
    def is_done(self) -> None:
        ...
        # reset if EE is at target position or max episode length is exceeded
        # self.reset_buf = torch.where(torch.norm(self.hand_pos - self.target_pos, p=2, dim=1) < 0.05, torch.ones_like(self.reset_buf), self.reset_buf)
