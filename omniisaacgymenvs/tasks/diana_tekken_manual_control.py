...
class DianaTekkenManualControlTask(DianaTekkenTask):
    ...
    def post_reset(self):
        self.joint_targets = np.zeros(22)
        super().post_reset()
    ...
    def pre_physics_step(self, actions: torch.tensor) -> None:
        ...
        target_pos[0, :3] += actions[0,:3] * 0.01
        rpy_target[0, 1] += actions[0,3] * 0.01
        ...
        if actions[0,-1] == 1:
            # Add the finger targets at the right indices
            self.joint_targets[self._robots.actuated_finger_dof_indices]= np.ones(len(self._robots.actuated_finger_dof_indices)) * np.pi/3
        else:
            self.joint_targets[self._robots.actuated_finger_dof_indices]= np.zeros(len(self._robots.actuated_finger_dof_indices))

        if succ:
            # If success store the ik targets to the targets
            self.joint_targets[self._robots.actuated_diana_dof_indices] = robot_actions.joint_positions
            unsqueezed_targets = (self.joint_targets.reshape(1,-1)).astype(np.float32)

            current_targets = np.array(self._robots.get_applied_actions().joint_positions[:, self.actuated_dof_indices])
            unsqueezed_targets[:, self.actuated_dof_indices] = (unsqueezed_targets[:, self.actuated_dof_indices] - current_targets)/(self.dt * self.action_scale)

            super().pre_physics_step(torch.tensor(unsqueezed_targets[:, self.actuated_dof_indices]))
        else:
